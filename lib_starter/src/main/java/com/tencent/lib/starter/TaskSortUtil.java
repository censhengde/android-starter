...
* 协调、排序Task*/
 final class TaskSortUtil {
     ...
     * @return
     */
     static synchronized List<Task> getSortResult(List<Task> originTasks,
                                                        List<Class<? extends Task>> clsLaunchTasks) {
        long makeTime = System.currentTimeMillis();

        Set<Integer> dependSet = new ArraySet<>();
        final Graph graph = new Graph(originTasks.size());
        ...
        StarterLog.i("task analyse cost makeTime " + (System.currentTimeMillis() - makeTime));
    ...
    private static void printAllTaskName(List<Task> newTasksAll) {
        ...
        for (Task task : newTasksAll) {
            StarterLog.i(task.getClass().getSimpleName());
        }
    }
